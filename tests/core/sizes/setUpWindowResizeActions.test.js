import { setUpWindowResizeActions } from "../../../src/core/sizes/setUpWindowResizeActions";
...
describe('setting up max source dimensions to data object', () => {
    describe('window.innerWidth < 1000', () => {
        ...
        it('should set max source width to window.innerWidth', () => {
            expect(fsLightbox.data.maxSourceWidth).toBe(window.innerWidth);
        });

        it('should set max source height to to 90% of window.innerHeight', () => {
    ...
    describe('window.innerWidth >= 1000', () => {
        ...
        it('should set max source width to 90% of window.innerWidth', () => {
            expect(fsLightbox.data.maxSourceWidth).toBe(0.9 * window.innerWidth);
        });

        it('should set max source height to 90% of window.innerHeight', () => {
            expect(fsLightbox.data.maxSourceHeight).toBe(0.9 * window.innerHeight);
        });
    });
});
