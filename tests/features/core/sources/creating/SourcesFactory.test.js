...
describe(`updating or not updating source holder, 
        if source holder is not mounted(e.g. lightbox is closed) source holder should not be updated,
        if source holder is mounted source holder should be updated`, () => {
    let firstSourceHolderState;
    let secondSourceHolderState;

    beforeAll(() => {
        firstSourceHolderState = {
            get: () => {},
            set: jest.fn()
        };
        secondSourceHolderState = {
            get: () => {},
            set: jest.fn()
        };
        fsLightbox.componentsStates.shouldSourceHolderBeUpdatedCollection = [
            firstSourceHolderState, secondSourceHolderState
        ];
        // first index is not set - sources holder should not be updated, second index is true - that one should be updated
        fsLightbox.sourcesData.isSourceHolderMountedArray = [undefined, true];
        // we set getSourceType to be instantly resolved (we are not testing it)
        sourceTypeGetter.getSourceType = () => new Promise(resolve => resolve('random sources type'));
        sourcesFactory = new SourcesFactory(fsLightbox);
        sourcesFactory.createSourcesAndAddThemToProperArrays();
    ...
    describe('first sources holder', () => {
        it('should not call set method on shouldSourceHolderBeUpdatedState', () => {
            expect(firstSourceHolderState.set).not.toBeCalled();
        });
    });

    describe('second sources holder (that one  should be updated)', () => {
        it('should all set method on shouldSourceHolderBeUpdatedState with true', () => {
            expect(secondSourceHolderState.set).toBeCalledWith(true);
        });
    });
});
